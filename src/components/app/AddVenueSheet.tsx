...
function slugify(s: string) {
  return s.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    .replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 60) || "loc";
}
...
export function AddVenueSheet({ cities, onAdded }: Props) {
  ...
  const [hours, setHours] = useState<OpeningHours>(() => {
    ...
    DAYS.forEach(d => { o[d.key] = { open: "18:00", close: "02:00" }; });
    return o;
  });
  const [saving, setSaving] = useState(false);

  const reset = () => {
    setOpen(false); setStep("loc"); setGeoState("idle"); setCoords(null);
    setName(""); setAddress(""); setPhone(""); setCityId("");
  };

  const requestLoc = () => {
    if (!navigator.geolocation) { setGeoState("err"); return; }
    setGeoState("loading");
    navigator.geolocation.getCurrentPosition(
      (p) => {
        ...
        if (cities.length) {
          const closest = [...cities].sort((a, b) => distKm(lat, lng, a.lat, a.lng) - distKm(lat, lng, b.lat, b.lng))[0];
  ...
  const setTime = (k: DayKey, field: "open" | "close", v: string) => {
    setHours(h => ({ ...h, [k]: { ...(h[k] ?? { open: "18:00", close: "02:00" }), [field]: v } }));
  };

  const save = async () => {
    if (!user) { toast.error("Trebuie să fii logat"); return; }
    if (!name.trim() || !cityId || !coords) { toast.error("Completează numele, orașul și locația"); return; }
    setSaving(true);
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 text-left">
          <div className="font-display font-black uppercase text-sm tracking-tight">adaugă local pe hartă</div>
      ...
      {open && (
        <div className="fixed inset-0 z-[100] bg-background/95 backdrop-blur-md flex flex-col">
          <div className="flex items-center justify-between p-4 border-b border-foreground/10">
            <div>
              <div className="font-display font-black text-xl">{step === "loc" ? "locație" : "detalii"}</div>
              <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
                pas {step === "loc" ? "1" : "2"}/2
              </div>
            </div>
            <button onClick={reset} className="h-10 w-10 rounded-full bg-foreground/10 flex items-center justify-center">
              <X size={18} />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {step === "loc" ? (
              <>
                ...
                >
                  ...
                  <div className="font-display font-black uppercase">
                    {geoState === "loading" ? "caut locația..."
                      : geoState === "ok" ? "locație prinsă"
                      : geoState === "err" ? "reîncearcă"
                      : "permite locația"}
                  </div>
                  {coords && (
                  ...
                  {geoState === "err" && (
                    <div className="text-xs text-neon-crimson">Refuzat. Activează GPS în setări.</div>
                  )}
                </button>
                <button
              ...
              <>
                <div>
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">nume *</label>
                  <input value={name} onChange={e => setName(e.target.value)} placeholder="ex: Berăria H"
                    className="w-full mt-1 px-3 py-3 rounded-xl bg-foreground/5 border border-foreground/15 font-display" />
                </div>
                <div>
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">tip</label>
                  <div className="grid grid-cols-5 gap-1.5 mt-1">
                    {TYPES.map(t => (
                      <button key={t.id} onClick={() => setType(t.id)}
                        className={`py-2 rounded-lg font-mono text-[10px] uppercase tracking-wider border ${
                          type === t.id ? "bg-neon-green/20 border-neon-green text-neon-green" : "bg-foreground/5 border-foreground/15"
                        }`}>{t.label}</button>
                    ))}
                  </div>
                </div>
                <div>
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">oraș *</label>
                ...
                <div>
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">adresă</label>
                  <input value={address} onChange={e => setAddress(e.target.value)} placeholder="opțional"
                    className="w-full mt-1 px-3 py-3 rounded-xl bg-foreground/5 border border-foreground/15 font-display" />
                </div>
                <div>
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">telefon</label>
                  <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="opțional" inputMode="tel"
                    className="w-full mt-1 px-3 py-3 rounded-xl bg-foreground/5 border border-foreground/15 font-display" />
                </div>

                <div>
                  <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-2">program</div>
                  <div className="space-y-1.5">
                    {DAYS.map(d => {
                      const s = hours[d.key];
                      return (
                        <div key={d.key} className="flex items-center gap-2 p-2 rounded-lg bg-foreground/5 border border-foreground/10">
                          ...
                          {s ? (
                            <>
                              <input type="time" value={s.open} onChange={e => setTime(d.key, "open", e.target.value)}
                                className="flex-1 px-2 py-1.5 rounded bg-background border border-foreground/15 font-mono text-xs" />
                              <span className="text-muted-foreground">→</span>
                              <input type="time" value={s.close} onChange={e => setTime(d.key, "close", e.target.value)}
                                className="flex-1 px-2 py-1.5 rounded bg-background border border-foreground/15 font-mono text-xs" />
                            </>
                          ) : (
                            <span className="flex-1 text-center font-mono text-[10px] uppercase text-muted-foreground">închis</span>
                          )}
                        </div>
                      );
                    })}
          ...
          {step === "info" && (
            <div className="p-4 border-t border-foreground/10 flex gap-2">
              <button onClick={() => setStep("loc")} className="px-4 py-3 rounded-xl bg-foreground/10 font-mono text-xs uppercase">
                ← înapoi
              </button>
