...
function milestoneCopy(m: Milestone): { title: string; sub: string; emoji: string } {
  switch (m) {
    case 3:   return { title: "3 săpt la rând", sub: "Te-ai prins. Continuă.", emoji: "🔥" };
    case 7:   return { title: "7 săpt streak", sub: "Asta deja înseamnă ceva.", emoji: "🔥🔥" };
    case 14:  return { title: "14 săpt streak", sub: "Nivel: imposibil de oprit.", emoji: "⚡" };
    case 30:  return { title: "30 săpt streak", sub: "Un sezon întreg. Legendă.", emoji: "👑" };
    case 100: return { title: "100 săpt streak", sub: "Te-au adăugat în manual.", emoji: "🪐" };
  }
}
...
export function writeSeenMilestone(m: number) {
  try { localStorage.setItem(SEEN_KEY, String(m)); } catch {}
}
...
export function StreakFlexSheet({
  ...
  const generatePng = async (): Promise<Blob | null> => {
    ...
    const dataUrl = await toPng(cardRef.current, { pixelRatio: 2, cacheBust: true, backgroundColor: "#0a0014" });
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (nav.canShare && nav.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: copy.title, text: `${copy.emoji} ${copy.title} pe Oxidații` });
      } else {
        triggerDownload(blob);
      }
    } catch { /* user cancelled */ } finally { setBusy(null); }
  };
    ...
    <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200 overflow-y-auto">
      ...
      <div className="flex flex-col items-center gap-4 my-auto">
        <div
          ...
          style={{
            background: "linear-gradient(170deg, #100018 0%, #2d0a1a 35%, #6b1300 70%, #ff6a00 120%)",
            boxShadow: "0 30px 80px -20px rgba(255,61,139,0.5)",
          }}
        >
          <div className="absolute -top-24 -right-20 w-96 h-96 rounded-full blur-3xl opacity-60"
               style={{ background: "radial-gradient(circle, #ff3d8b, transparent 70%)" }} />
          <div className="absolute -bottom-24 -left-24 w-96 h-96 rounded-full blur-3xl opacity-50"
          ...
          <div className="relative h-full flex flex-col p-7">
            ...
            <div className="mt-10 flex items-center gap-3">
              <Flame size={48} className="text-[#ff7a3d] drop-shadow-[0_0_18px_#ff3d8b]" fill="currentColor" />
            ...
            <div className="mt-8 space-y-2">
              ...
              <h1 className="text-[42px] leading-[0.95] text-white" style={{ fontFamily: "'Instrument Serif', serif" }}>
