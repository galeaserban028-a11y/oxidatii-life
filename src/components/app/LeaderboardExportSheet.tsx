...
export function LeaderboardExportSheet({ top3, me, scopeLabel, monthLabel, onClose }: Props) {
  ...
  const generatePng = async (): Promise<Blob | null> => {
    ...
    const dataUrl = await toPng(cardRef.current, { pixelRatio: 2, cacheBust: true, backgroundColor: "#04040a" });
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (nav.canShare && nav.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: `Top ${scopeLabel}`, text: `🏆 Top ${scopeLabel} · ${monthLabel} · Oxidații` });
      } else {
        triggerDownload(blob);
      }
    } catch { /* cancelled */ } finally { setBusy(null); }
  };

  const handleDownload = async () => {
    setBusy("download");
    try { const b = await generatePng(); if (b) triggerDownload(b); } finally { setBusy(null); }
  };
    ...
    <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto animate-in fade-in duration-200">
      ...
      <div className="flex flex-col items-center gap-4 my-auto">
        <div
          ...
          style={{
            background: "linear-gradient(170deg, #04040a 0%, #1a0633 35%, #4a0a44 70%, #c724ff 130%)",
          }}
        >
          <div className="absolute -top-24 -right-24 w-96 h-96 rounded-full blur-3xl opacity-50"
               style={{ background: "radial-gradient(circle, #c724ff, transparent 70%)" }} />
          <div className="absolute -bottom-24 -left-24 w-96 h-96 rounded-full blur-3xl opacity-40"
          ...
          <div className="relative h-full flex flex-col p-7">
            ...
            <div className="mt-6">
              <h1 className="text-[44px] leading-[0.95] text-white" style={{ fontFamily: "'Instrument Serif', serif" }}>
                <span className="italic">Spritz<br />Score</span>
              </h1>
            </div>
              ...
              {podium.map((p, i) => {
                ...
                return (
                  <div key={p.user_id} className="flex flex-col items-center gap-1.5">
                    <div className={`relative ${isKing ? "size-16" : "size-14"} rounded-full p-[2px] bg-gradient-to-br ${isKing ? "from-[#ffea00] to-[#ff3d8b]" : "from-white/30 to-white/5"}`}>
                      <div className="h-full w-full rounded-full overflow-hidden bg-black flex items-center justify-center text-white font-bold">
                        {p.avatar_url
                          ? <img src={p.avatar_url} alt="" crossOrigin="anonymous" className="h-full w-full object-cover" />
                          : <span>{handle[0]?.toUpperCase()}</span>}
                      </div>
                      {isKing && <div className="absolute -top-3 left-1/2 -translate-x-1/2 text-xl">👑</div>}
                    </div>
                    <div className="text-[11px] font-semibold text-white text-center w-full truncate">@{handle}</div>
                    <div className={`leading-none ${isKing ? "text-2xl text-[#ffea00]" : "text-lg text-white/80"} font-black tabular-nums`}>
                      {p.spritz_score}
                    </div>
                    <div className={`${h} w-full mt-1 rounded-t-xl ${isKing ? "bg-gradient-to-t from-[#c724ff]/60 to-transparent" : "bg-white/10"} flex items-start justify-center pt-1.5`}>
                      <span className={`text-[10px] font-mono font-bold ${isKing ? "text-[#ffea00]" : "text-white/50"}`}>#{realRank}</span>
                    </div>
                  </div>
                );
              })}
            ...
            {me && (
              <div className="mt-6 rounded-2xl border border-white/15 bg-white/[0.07] backdrop-blur p-3 flex items-center justify-between">
                <div>
                  <div className="text-[10px] font-mono uppercase tracking-widest text-white/50">Tu</div>
                  <div className="text-[18px] font-black text-white">
                    #{me.rank} {me.handle ? <span className="text-white/60 text-[13px] font-semibold">@{me.handle}</span> : null}
                  </div>
                </div>
                <div className="text-2xl font-black tabular-nums text-[#ffea00]">{me.spritz_score}</div>
              </div>
            )}
        ...
        <div className="w-full max-w-[380px] grid grid-cols-2 gap-2">
          ...
          <button onClick={handleDownload} disabled={busy !== null}
            ...
            {busy === "download" ? <Loader2 size={16} className="animate-spin" /> : <Download size={16} />} Salvează
          </button>
        </div>
      </div>
    </div>
