...
export default function VenueNightChat({
      ...
      .on(
        ...
        async (payload) => {
          ...
          const { data: prof } = await supabase
            ...
            .maybeSingle();
          setMsgs((prev) =>
            prev.some((m) => m.id === row.id) ? prev : [...prev, { ...row, author: prof }],
          );
  ...
  async function send() {
    ...
    try {
      ...
      if (data) {
        setMsgs((prev) =>
          prev.some((m) => m.id === data.id) ? prev : [...prev, { ...(data as Msg), author: null }],
        );
        requestAnimationFrame(() =>
          scrollRef.current?.scrollTo({ top: 999999, behavior: "smooth" }),
        );
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
          <div className="min-w-0">
            <div className="text-[10px] font-mono uppercase tracking-[0.25em] text-[#ffea00]">
              chat diseară
            </div>
          ...
          <button
            onClick={onClose}
            className="h-9 w-9 rounded-full bg-white/5 border border-white/10 text-white/70 flex items-center justify-center"
          >
            <X size={16} />
          </button>
        </div>
            ...
            return (
              <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"} gap-2`}>
                ...
                <div className="max-w-[75%] flex flex-col">
                  <div
                    className={`text-[10px] px-1 mb-0.5 ${mine ? "text-right text-white/50" : "text-white/50"}`}
                  >
                  ...
                  <div
                    className={`rounded-2xl px-3 py-2 ${mine ? "bg-[#ffea00] text-black" : "bg-white/[0.07] text-white"}`}
                  >
                    <div className="text-[14px] leading-snug whitespace-pre-wrap break-words">
                      {m.body}
                    </div>
                  </div>
                </div>
        ...
        <div className="border-t border-white/10 p-2 flex gap-2">
          ...
          <button
            ...
            className="h-10 w-10 rounded-full bg-[#ffea00] text-black flex items-center justify-center disabled:opacity-40"
          >
            <Send size={16} />
          </button>
        </div>
