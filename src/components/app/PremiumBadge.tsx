...
const TIER_META: Record<string, { label: string; icon: typeof Crown; cls: string; glow: string }> =
  {
    vip: {
      label: "VIP",
      icon: Star,
      cls: "from-amber-300 to-amber-500 text-black",
      glow: "shadow-[0_0_12px_rgba(251,191,36,0.55)]",
    },
    vip_plus: {
      label: "VIP+",
      icon: Sparkles,
      cls: "from-pink-400 to-fuchsia-500 text-white",
      glow: "shadow-[0_0_14px_rgba(236,72,153,0.6)]",
    },
    pro: {
      label: "PRO",
      icon: Crown,
      cls: "from-violet-500 to-indigo-600 text-white",
      glow: "shadow-[0_0_16px_rgba(139,92,246,0.7)]",
    },
    elite: {
      label: "ELITE",
      icon: Gem,
      cls: "from-cyan-300 via-fuchsia-400 to-amber-300 text-black",
      glow: "shadow-[0_0_22px_rgba(244,114,182,0.8)]",
    },
  };
...
export function PremiumBadge({
  ...
  const Inner = (
    <span
      className={`inline-flex items-center gap-1 rounded-full font-mono font-bold uppercase tracking-wider bg-gradient-to-r ${meta.cls} ${meta.glow} ${px}`}
    >
  ...
  return asLink ? (
    <Link to="/app/premium" className="active:scale-95 transition">
      {Inner}
    </Link>
  ) : (
    Inner
