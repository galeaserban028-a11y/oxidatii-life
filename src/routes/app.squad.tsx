  ...
  return (
    <div className="pb-4" style={HIND}>
      ...
      <div className="px-6 mt-5 mb-7 grid grid-cols-3 gap-2">
        <a
          href="#live"
          className="bg-white/5 rounded-2xl py-3 border border-white/5 flex flex-col items-center active:scale-95 transition"
        >
          <span className="text-xl text-[#ff3d8b]" style={ARCHIVO}>
            {openCount}
          </span>
          <span className="text-[8px] uppercase tracking-widest text-white/30 font-bold mt-1">
            live
          </span>
        </a>
        <a
          ...
          className="bg-white/5 rounded-2xl py-3 border border-white/5 flex flex-col items-center active:scale-95 transition"
        >
          <span className="text-xl text-[#c724ff]" style={ARCHIVO}>
            {groupCount}
          </span>
          <span className="text-[8px] uppercase tracking-widest text-white/30 font-bold mt-1">
            găști
        ...
        <a
          ...
          className="bg-white/5 rounded-2xl py-3 border border-white/5 flex flex-col items-center active:scale-95 transition"
        >
          <span className="text-xl text-[#00e5ff]" style={ARCHIVO}>
            {friendCount}
          </span>
          <span className="text-[8px] uppercase tracking-widest text-white/30 font-bold mt-1">
            haita
      ...
      <div className="px-6 grid grid-cols-2 gap-4 mb-10">
        ...
        >
          ...
          <p className="text-xs uppercase mb-1 tracking-tight text-white" style={ARCHIVO}>
            deschide unul
          </p>
        ...
        >
          ...
          <div className="flex-1 text-left">
            ...
            <p className="text-[10px] text-white/50 mt-0.5">
              vot rapid pentru grup • sugestii live
            </p>
          </div>
      ...
      <section id="live" className="px-6 mb-10 scroll-mt-4">
        <div className="flex items-center justify-between mb-5">
          ...
          {openCount > 0 && (
            <Link
              to="/app/parties"
              className="text-[10px] text-white/30 uppercase font-bold tracking-wider"
            >
              vezi tot →
            </Link>
          )}
        ...
        {visibleParties.length === 0 ? (
          ...
          >
            ...
            <div className="text-sm text-white" style={ARCHIVO}>
              nimic deschis acum
            </div>
          ...
          <div className="space-y-4">
            {visibleParties.map((p) => {
              ...
              const joined =
                !!user && joins.some((j) => j.party_id === p.id && j.user_id === user.id);
              const isHost = user?.id === p.host_id;
                      ...
                      <div className="mt-6 flex items-end justify-between gap-3">
                        <div className="flex items-center gap-2 min-w-0">
                          <div className="bg-white/5 px-3 py-1.5 rounded-lg border border-white/5">
                            <span className="text-white/40 text-[9px] font-black uppercase tracking-widest block leading-none">
                              Locuri
                            </span>
                            <span
                              className={`text-sm font-black tracking-tight ${full ? "text-[#c724ff]" : "text-white"}`}
                            >
                              {taken}
                        ...
                        {isHost ? (
                          <Link to="/app/parties" className="group/btn relative shrink-0">
      ...
      {groups.length > 0 && (
        <section id="groups" className="px-6 mb-10 scroll-mt-4">
          <h2 className="text-xs uppercase tracking-widest text-[#c724ff] mb-5" style={ARCHIVO}>
            grupuri active
          </h2>
            ...
            {(groups as Array<{ id: string; title: string | null; last_message_at: string | null }>).map((g) => (
              ...
              >
                <div className="flex items-center gap-4 min-w-0">
                  <div
                    className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#c724ff] to-[#00e5ff] flex items-center justify-center text-sm text-white shrink-0"
                    style={ARCHIVO}
                  >
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-bold text-white truncate">
                      {g.title ?? "grup fără nume"}
                    </p>
                    <p className="text-[10px] text-white/40">
                      {g.last_message_at
                        ? new Date(g.last_message_at).toLocaleDateString("ro-RO", {
                            day: "numeric",
                            month: "short",
                          })
                        : "—"}
          ...
          <div className="divide-y divide-white/5">
            {(friends as Array<{ id: string; handle: string | null; display_name: string | null; avatar_url: string | null; city?: { name: string } | null }>).map((p) => (
              <div key={p.id} className="flex items-center justify-between py-3">
                ...
                >
                  <div
                    className="w-10 h-10 rounded-full overflow-hidden bg-gradient-to-br from-[#ff3d8b] to-[#00e5ff] flex items-center justify-center text-sm text-white shrink-0"
                    style={ARCHIVO}
                  >
                    {p.avatar_url ? (
