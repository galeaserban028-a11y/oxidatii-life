  ...
  return (
    <main className="min-h-[100svh] bg-[#050510] text-white px-5 pt-4 pb-24">
      ...
      <div className="text-center mb-6">
        <Crown
          className={`w-12 h-12 mx-auto mb-2 ${stats?.founding_unlocked ? "text-amber-400 drop-shadow-[0_0_20px_rgba(251,191,36,0.7)]" : "text-white/30"}`}
        />
      ...
      <div className="rounded-3xl border border-amber-400/30 bg-gradient-to-br from-amber-500/15 via-fuchsia-500/5 to-transparent p-6 mb-4">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="font-display font-black text-4xl tracking-widest tabular-nums">
            {stats?.code || "—"}
          </div>
        ...
        <button
          onClick={share}
          className="mt-4 w-full py-3 rounded-full bg-gradient-to-r from-amber-500 to-pink-500 text-black font-display font-bold uppercase tracking-widest text-sm flex items-center justify-center gap-2"
        >
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-5 mb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="font-mono text-sm">
            {stats?.count ?? 0} / {stats?.next_milestone ?? "∞"}
          </span>
        </div>
        ...
        <div className="mt-5 space-y-3">
          <Milestone
            done={!!stats && stats.count >= 1}
            title="Primul invitat"
            subtitle="+25 șprițuri / invitație"
          />
          <Milestone
            done={!!stats?.founding_unlocked}
            title="3 invitați → Ramă Founding Member"
            subtitle="Exclusivă. Nu se mai poate cumpăra niciodată."
            icon={<Sparkles className="w-4 h-4" />}
          />
          <Milestone
            done={!!stats?.hall_of_fame}
            title="10 invitați → Hall of Fame"
            subtitle="Numele tău public pe oxidatii.life/hall-of-fame, forever."
          />
        </div>
        ...
        {stats?.rank && (
          <div className="mt-4 pt-4 border-t border-white/10 text-center text-sm">
            Ești pe locul{" "}
            <span className="font-display font-black text-amber-400">#{stats.rank}</span> în
            clasamentul de invitatori
          </div>
        )}
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-5 text-sm text-white/70 space-y-2">
        ...
        <p>
          3. Tu primești <b>25 șprițuri</b>, ei primesc <b>50</b>.
        </p>
        <p>
          4. La 3 invitați primești rama <b>Founding Member</b> permanent.
        </p>
      ...
      <a
        href="/hall-of-fame"
        target="_blank"
        className="mt-4 block text-center text-xs text-amber-400 underline"
      >
        Vezi Hall of Fame →
      </a>
    </main>
...
function Milestone({
  ...
  return (
    <div
      className={`flex items-start gap-3 p-3 rounded-xl border ${done ? "border-amber-400/40 bg-amber-500/5" : "border-white/10 bg-transparent opacity-60"}`}
    >
      <div
        className={`mt-0.5 w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${done ? "bg-amber-400 text-black" : "bg-white/10 text-white/50"}`}
      >
        {done ? "✓" : (icon ?? "•")}
      </div>
      <div>
