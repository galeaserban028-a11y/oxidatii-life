...
function PartiesPage() {
  ...
  const { data: joins = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        .from("party_joins")
        .select("id, party_id, user_id, status, created_at")
  ...
  const joinMutation = useMutation({
    mutationFn: async ({ partyId, joinId }: { partyId: string; joinId: string | null }) => {
      ...
      if (joinId) {
        ...
        if (!(await guardRateLimit("party_join")))
          throw new Error("Prea multe cereri. Așteaptă un minut.");
        await supabase
          .from("party_joins")
          .insert({ party_id: partyId, user_id: user.id, status: "pending" });
        try {
  ...
  const acceptedFor = (id: string) =>
    joins.filter((j) => j.party_id === id && j.status === "accepted").length;
    ...
    <div className="pb-4" style={HIND}>
      ...
      <div className="px-6 mt-5 mb-8">
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm uppercase tracking-tight text-white" style={ARCHIVO}>
              deschide un șpriț
            </p>
      ...
      <section className="px-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h2 className="text-xs uppercase tracking-widest text-[#ff3d8b]" style={ARCHIVO}>
            live acum
          </h2>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="p-8 rounded-3xl border border-dashed border-white/10 bg-white/[0.02] text-center">
            ...
            <div className="text-base text-white" style={ARCHIVO}>
              zero șprițuri acum
            </div>
            <div className="text-[10px] text-white/40 mt-1 uppercase tracking-widest">
              fii primul · deschide unul
            </div>
            <button
              onClick={() => setShowCreate(true)}
              disabled={!user}
            ...
            {visibleParties.map((p) => {
              ...
              return (
                <article key={p.id} className="relative">
                  ...
                  <div
                    className={`relative overflow-hidden bg-[#0f0f12] border border-white/10 rounded-[2rem] p-6 ${isHost ? "pl-8" : ""} shadow-xl`}
                  >
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex gap-4 min-w-0 flex-1">
                          <div className="relative shrink-0">
                            <div
                              className="w-14 h-14 rounded-2xl overflow-hidden bg-gradient-to-br from-[#ff3d8b] to-[#00e5ff] flex items-center justify-center text-base text-white ring-1 ring-white/10 shadow-xl"
                              style={ARCHIVO}
                            >
                              {host?.avatar_url ? (
                                <img
                          ...
                          <div className="pt-1 min-w-0 flex-1">
                            ...
                            <p className="text-white/40 text-[11px] mt-0.5 flex items-center gap-1.5 truncate">
                              <span
                                className={`font-bold uppercase tracking-wide ${isHost ? "text-[#00e5ff]" : "text-white/60"}`}
                              >
                                {isHost
                                  ? "Tu (Host)"
                        ...
                        {isHost ? (
                          ...
                          <div className="px-2 py-1 bg-[#ff3d8b]/10 border border-[#ff3d8b]/20 rounded-lg shrink-0 flex items-center gap-1">
                            ...
                            <p className="text-[10px] font-bold text-[#ff3d8b]">
                      ...
                      <div className="mt-5">
                        <div className="flex items-center justify-between mb-1.5">
                          ...
                          <span
                            className={`text-[10px] uppercase tracking-widest font-black ${free === 0 ? "text-[#c724ff]" : "text-white/40"}`}
                          >
                        ...
                        {isHost ? (
                          <div className="group/btn relative">
                            ...
                            <div className="relative bg-gradient-to-r from-[#ff3d8b] to-[#c724ff] px-5 py-3 rounded-2xl flex items-center gap-2 shadow-2xl">
                              <span className="text-white text-[11px] font-black tracking-[0.12em] uppercase">
                                Gestionezi
                              </span>
                              <svg
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() =>
                                joinMutation.mutate({ partyId: p.id, joinId: myJoin?.id ?? null })
                              }
    ...
    <div className="rounded-2xl border border-white/5 bg-black/40 overflow-hidden">
      ...
      >
        <span className="flex items-center gap-2">
          ...
          {pending.length > 0 && (
            <span className="px-1.5 py-0.5 rounded-full bg-[#ff3d8b] text-black text-[9px] font-bold">
              {pending.length} noi
            </span>
          )}
        ...
        <div className="px-3 pb-3 space-y-2.5">
          {pending.map((j) => {
            ...
            return (
              <div key={j.id} className="flex items-center gap-2.5">
                ...
                >
                  <div
                    className="h-8 w-8 rounded-lg overflow-hidden bg-gradient-to-br from-[#00e5ff] to-[#ff3d8b] flex items-center justify-center text-[10px] text-white shrink-0"
                    style={ARCHIVO}
                  >
                    {u?.avatar_url ? (
                  ...
                  <div className="min-w-0">
                    <div className="text-xs font-bold text-white truncate group-hover:underline">
                      @{u?.handle ?? u?.display_name ?? "anonim"}
                    </div>
          ...
          {accepted.length > 0 && (
            <>
              ...
              <div className="text-[9px] uppercase tracking-widest text-[#00e5ff] font-bold">
                vin
              </div>
              {accepted.map((j) => {
                const u = profileMap.get(j.user_id);
                return (
                  <div key={j.id} className="flex items-center gap-2.5">
                    ...
                    >
                      <div
                        className="h-8 w-8 rounded-lg overflow-hidden bg-gradient-to-br from-[#00e5ff] to-[#c724ff] flex items-center justify-center text-[10px] text-white shrink-0"
                        style={ARCHIVO}
                      >
                        {u?.avatar_url ? (
                          <img src={u.avatar_url} alt="" className="h-full w-full object-cover" />
...
function CreatePartySheet({ onClose }: { onClose: () => void }) {
    ...
    mutationFn: async () => {
      ...
      if (inserted?.id) {
        ...
        if (payErr) {
          ...
          throw new Error(
            /insufficient/i.test(payErr.message)
              ? `n-ai destui coins (ai ${balance}, trebuie ${PARTY_COST})`
              : payErr.message,
          );
        }
        try {
  ...
  const inputCls =
    "w-full px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-[#ff3d8b]";
      ...
      >
        <div className="sticky top-0 z-10 -mx-5 -mt-5 px-5 pt-5 pb-3 bg-[#0a0a0a]/95 backdrop-blur-xl border-b border-white/5 flex items-center justify-between">
          <div>
            ...
            <h2 className="text-2xl text-white mt-0.5" style={ARCHIVO}>
              deschid un șpriț.
            </h2>
          </div>
          <button
            onClick={onClose}
            className="h-9 w-9 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/60"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="space-y-1.5">
            ...
            <input
              ...
              max={99}
              value={spots}
              onChange={(e) => setSpots(Math.max(1, Math.min(99, +e.target.value || 1)))}
              className={inputCls}
            />
          </div>
          <div className="space-y-1.5">
        ...
        <div className="sticky bottom-0 z-10 -mx-5 -mb-5 px-5 pt-3 pb-5 bg-[#0a0a0a]/95 backdrop-blur-xl border-t border-white/5">
          ...
          >
            {create.isPending ? (
              <span className="text-[11px] uppercase tracking-[0.3em] text-white/80 font-bold">
                se deschide…
              </span>
            ) : (
              <div className="flex flex-col items-center gap-1">
                <span
                  className={`text-sm uppercase tracking-[0.18em] ${isDisabled ? "text-white/40" : "text-black"}`}
                  style={ARCHIVO}
                >
                  deschide șprițul 🥂
                </span>
                <span
                  className={`text-[10px] uppercase tracking-[0.25em] flex items-center gap-1.5 font-bold ${
