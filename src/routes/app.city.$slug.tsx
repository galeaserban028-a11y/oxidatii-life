...
function CityPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data: city, error } = await supabase
        .from("cities").select("id,name,slug,region,chaos_level").eq("slug", slug).single();
      if (error) throw error;
      const { data: streets } = await supabase
        .from("streets").select("id,name,slug,venues:venues(count)")
        .eq("city_id", city.id).order("name");
  ...
  return (
    <div className="px-4 pt-6 pb-4 space-y-4">
      <Link to="/app/map" className="inline-flex items-center gap-1 text-xs font-mono uppercase tracking-widest text-muted-foreground">
        <ArrowLeft size={14} /> înapoi
      </Link>
      <div>
        <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-neon-green">{data.city.region}</div>
        <h1 className="font-display font-black text-4xl">{data.city.name}</h1>
        <div className="font-mono text-xs text-muted-foreground mt-1">chaos {Number(data.city.chaos_level).toFixed(1)} · {data.streets.length} străzi active</div>
      </div>

      <div>
        <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-2">// străzi</div>
        <div className="space-y-2">
          {data.streets.map((s: any) => {
            const count = s.venues?.[0]?.count ?? 0;
            return (
              <Link key={s.id} to="/app/street/$id" params={{ id: s.id }}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-[10px] font-mono uppercase tracking-widest text-neon-purple">{count} venue{count !== 1 ? "s" : ""}</div>
                </div>
              </Link>
            );
          })}
