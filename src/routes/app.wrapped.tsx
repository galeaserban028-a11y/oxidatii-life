...
function WrappedPage() {
  ...
  const share = async () => {
    ...
    try {
      if (navigator.share) await navigator.share({ title: "Wrapped OXIDAȚII", text, url: "https://oxidatii.life" });
      else { await navigator.clipboard.writeText(text); toast.success("Copiat"); }
    } catch {}
  };
  ...
  return (
    <main className="min-h-[100svh] bg-[#050510] text-white px-5 pt-4 pb-24">
      ...
      <div className="text-center mb-4">
        <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-fuchsia-400 mb-1">// WRAPPED</div>
        <h1 className="font-display font-black text-2xl">{monthName} {year}</h1>
      </div>

      {/* Shareable card */}
      <div ref={cardRef} className="relative rounded-[2rem] overflow-hidden border border-white/10 aspect-[9/16] max-h-[70vh] mx-auto" style={{ background: "linear-gradient(160deg, #1a0633 0%, #3d0a4d 40%, #6b0840 80%, #ff6b00 100%)" }}>
        {/* glows */}
        ...
        <div className="relative h-full flex flex-col p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="font-mono text-[9px] uppercase tracking-[0.3em] text-white/60">OXIDAȚII WRAPPED</div>
            <div className="font-mono text-[9px] uppercase tracking-widest text-white/60">{monthName} '{String(year).slice(2)}</div>
          </div>

          {profile?.display_name && (
            <div className="flex items-center gap-2 mb-6">
              {profile.avatar_url && <img src={profile.avatar_url} alt="" className="w-10 h-10 rounded-full border-2 border-white/30" />}
          ...
          <div className="flex-1 flex flex-col justify-center">
            ...
            <div className="text-xs uppercase tracking-[0.3em] text-white/70 mt-2 mb-6">șprițuri băute</div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              ...
              <div className="rounded-xl bg-white/10 backdrop-blur p-3">
                <div className="font-display font-black text-sm truncate">{wrap?.top_venue?.name || "—"}</div>
            ...
            {wrap?.top_crew && wrap.top_crew.length > 0 && (
              <div className="mb-4">
                <div className="text-[10px] uppercase tracking-widest text-white/60 mb-2">Crew-ul tău</div>
                <div className="flex -space-x-2">
                  {wrap.top_crew.slice(0, 5).map((m) =>
                    m.avatar_url ? (
                      <img key={m.id} src={m.avatar_url} alt="" className="w-9 h-9 rounded-full border-2 border-[#3d0a4d]" />
                    ) : (
                      <div key={m.id} className="w-9 h-9 rounded-full bg-white/20 border-2 border-[#3d0a4d]" />
                    ),
                  )}
                </div>
              </div>
      ...
      <div className="mt-5 flex gap-2 max-w-md mx-auto">
        <button onClick={share} className="flex-1 py-3 rounded-full bg-gradient-to-r from-fuchsia-500 to-orange-500 font-display font-bold uppercase tracking-widest text-sm flex items-center justify-center gap-2">
          <Share2 className="w-4 h-4" /> Share
        </button>
        <button onClick={() => toast.info("Screenshot manual pentru moment 📸")} className="px-4 py-3 rounded-full bg-white/10 border border-white/20">
