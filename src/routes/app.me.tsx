    ...
    queryFn: async () => {
      ...
      const [photosRes, proofsRes, cityRes] = await Promise.all([
        ...
        supabase
          .from("sprit_proofs")
          .select(
            "id, photo_url, created_at, ai_verified, venue:venues(name, id, city:cities(name))",
          )
          .eq("user_id", user.id)
    ...
    <div className="relative pb-3 bg-[#050505] min-h-screen text-white overflow-hidden">
      ...
      >
        ...
        >
          <Link
            to="/app/scan"
            className="p-1.5 -ml-1.5 active:scale-95 transition text-white/80 hover:text-[#ffea00]"
            aria-label="Adaugă"
          >
          ...
          <Link
            to="/app/settings"
            className="p-1.5 active:scale-95 transition text-white/80 hover:text-white"
            aria-label="Setări"
          >
          ...
          <Sheet open={menuOpen} onOpenChange={setMenuOpen}>
            <SheetTrigger asChild>
              <button
                className="p-1.5 -mr-1.5 active:scale-95 transition relative text-white/80"
                aria-label="Meniu"
              >
            ...
            <SheetContent
              side="right"
              className="w-[280px] sm:w-[320px] p-0 bg-[#0a0a0a] border-l border-white/10 text-white"
            >
              <SheetHeader className="px-4 py-3 border-b border-white/10 text-left">
                <SheetTitle style={instrument} className="text-2xl text-white">
                  Meniu<span className="text-[#ffea00]">.</span>
                </SheetTitle>
              </SheetHeader>
              <nav className="py-2">
                <MenuItem
                  to="/app/settings"
                  icon={<Settings size={16} className="text-[#ffea00]" />}
                  onSelect={() => setMenuOpen(false)}
                  label="Setări"
                />
                <MenuItem
                  to="/app/premium"
                  icon={<Gem size={16} className="text-[#c724ff]" />}
                  onSelect={() => setMenuOpen(false)}
                  label="Șpriț Premium ✨"
                />
                {ent.isVipPlus && (
                  <MenuItem
                    to="/app/me/raters"
                    icon={<Gem size={16} className="text-rose-400" />}
                    onSelect={() => setMenuOpen(false)}
                    label="Cine ți-a dat rating"
                  />
                )}
                {ent.isPro && (
                  <MenuItem
                    to="/app/me/reputation"
                ...
                <MenuItem
                  to="/app/biz"
                  icon={<Rocket size={16} className="text-[#00e5ff]" />}
                  onSelect={() => setMenuOpen(false)}
                  label="Business · Promovare"
                />
                {isStaff && (
                  <MenuItem
                    to="/app/admin"
                    icon={<ShieldAlert size={16} className="text-[#c724ff]" />}
                    onSelect={() => setMenuOpen(false)}
                    label={isAdmin ? "Panou Admin" : "Panou Moderator"}
                  />
                )}
                <div className="my-1 border-t border-white/10" />
                <MenuItem
                  to="/app/me/archive"
                  icon={<Bookmark size={16} className="text-[#ffea00]" />}
                  onSelect={() => setMenuOpen(false)}
                  label="Arhiva ta"
                />
                <MenuItem
                ...
                <MenuItem
                  to="/app/requests"
                  icon={<UserPlus size={16} className="text-white/70" />}
                  onSelect={() => setMenuOpen(false)}
                  label="Cereri urmărire"
                  badge={pendingCount > 0 ? pendingCount : undefined}
                />
                <MenuItem
                  to="/app/blocked"
                  icon={<ShieldOff size={16} className="text-white/70" />}
                  onSelect={() => setMenuOpen(false)}
                  label="Utilizatori blocați"
                />
                <MenuItem
                  to="/app/inbox"
                  icon={<UserSquare2 size={16} className="text-white/70" />}
                  onSelect={() => setMenuOpen(false)}
                  label="Mesaje"
                />
                ...
                >
                  {profile.is_public ? (
                    <Globe2 size={16} className="text-[#ffea00]" />
                  ) : (
                    <Lock size={16} className="text-[#c724ff]" />
                  )}
                  <span className="text-sm flex-1">
                    Cont:{" "}
                    <span className={profile.is_public ? "text-[#ffea00]" : "text-[#c724ff]"}>
        ...
        <div className="px-5 pt-6">
          ...
          <div className="flex items-center gap-6">
            ...
            >
              ...
              {uploading && (
                <div className="absolute inset-0 rounded-full bg-black/60 flex items-center justify-center text-[10px] font-mono uppercase z-10">
                  …
                </div>
              )}
            </button>
            ...
            >
              ...
              <div className="relative flex-1 min-w-0">
                <div style={instrument} className="text-lg leading-tight text-white">
                  Devino <em className="text-[#ffea00] not-italic">VIP, PRO sau ELITE</em>
                </div>
                <div className="text-[10px] text-white/50 mt-0.5">
                  Badge, frame, teme, șprițuri · de la 2.99 lei
                </div>
              </div>
              <span className="relative text-[10px] font-mono uppercase tracking-wider text-white/50 group-hover:text-[#ffea00] transition-colors">
                Vezi →
              </span>
            </Link>
          )}
              ...
              ent.isActive && (
                ...
                >
                  ...
                  <div className="flex-1 text-left">
                    <div style={instrument} className="text-base text-white leading-tight">
                      Editează profil premium
                    </div>
                    <div className="text-[10px] font-mono uppercase tracking-widest text-white/40 mt-0.5">
                      temă · muzică · background
                    </div>
                  </div>
          ...
          <div className="mt-8 grid grid-cols-2 gap-3">
            ...
            >
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#c724ff]/20 to-[#ff3d8b]/10 flex items-center justify-center">
                <svg
                  className="w-5 h-5 text-[#c724ff] group-hover:scale-110 transition-transform"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  viewBox="0 0 24 24"
              ...
              <div>
                <div style={instrument} className="text-lg leading-none">
                  Caută oameni
                </div>
            ...
            >
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#ffea00]/20 to-[#ff3d8b]/10 flex items-center justify-center">
                <Gem
                  size={18}
                  className="text-[#ffea00] group-hover:scale-110 transition-transform"
                />
              </div>
              <div>
                <div style={instrument} className="text-lg leading-none">
                ...
                <div className="text-[10px] text-white/40 mt-1 font-mono uppercase tracking-wider">
                  ...
                  {(profile.coin_balance ?? 0) === 1 ? "șpriț" : "șprițuri"} · dă rândul
                </div>
              </div>
            </Link>
          </div>
        ...
        <div className="grid grid-cols-3">
          ...
          >
            ...
            {tab === "posts" && (
              <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-12 h-[2px] bg-gradient-to-r from-[#ff3d8b] to-[#c724ff]" />
            )}
          </button>
          <button
          ...
          >
            ...
            {tab === "spritz" && (
              <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-12 h-[2px] bg-gradient-to-r from-[#ffea00] to-[#ff3d8b]" />
            )}
          </button>
          <button
        ...
        <FadeIn key={tab} y={10}>
          ...
          {tabMoments.length === 0 ? (
            <div className="mx-4 mt-4 rounded-3xl border border-white/10 bg-[#0d0d0d] p-8 text-center space-y-3">
              ...
              <Link
                to={tab === "reposts" ? "/app/faze" : "/app/scan"}
                className="inline-flex mt-2 font-mono text-[10px] uppercase tracking-widest px-4 py-2 rounded-full bg-gradient-to-r from-[#ff3d8b] to-[#c724ff] text-white"
              >
                {tab === "reposts"
                  ? "vezi fazele →"
                  : tab === "spritz"
                      ...
                      {isVideo ? (
                        <>
                          ...
                          <div className="absolute top-1 left-1 px-1.5 py-0.5 rounded-md bg-black/60 border border-white/20 flex items-center gap-1">
                            ...
                            <span className="font-mono text-[8px] uppercase text-white tracking-wider">
                              video
                            </span>
                          </div>
                        </>
...
function Stat({ label, value, color }: { label: string; value: number | string; color: string }) {
  return (
    <div className="rounded-xl bg-background/60 border border-foreground/10 p-3 text-center">
      <div className="font-display text-2xl" style={{ color, textShadow: `0 0 10px ${color}` }}>
        {value}
      </div>
