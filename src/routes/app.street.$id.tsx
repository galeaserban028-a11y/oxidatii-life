...
function StreetPage() {
  ...
  return (
    <div className="px-4 pt-6 pb-4 space-y-5">
      ...
      <div>
        <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-neon-green/80">teritoriu activ · {data.venues.length} locuri</div>
        ...
        {data.venues.map((v: any) => (
          <Link key={v.id} to="/app/venue/$id" params={{ id: v.id }}
            ...
            <div className="h-44 bg-gradient-to-br from-neon-purple/30 via-neon-crimson/20 to-neon-blue/30 relative">
              {v.cover_url && <img src={v.cover_url} alt={v.name} className="h-full w-full object-cover" loading="lazy" />}
              ...
              <div className="absolute inset-x-0 bottom-0 p-3 flex items-end justify-between gap-2">
                <div className="font-display font-black text-xl leading-tight text-white drop-shadow">{v.name}</div>
                {v.ig_handle && (
            ...
            {v.description && (
              <div className="px-3 py-2.5 text-xs text-muted-foreground line-clamp-2">{v.description}</div>
            )}
          </Link>
        ))}
      </div>
