...
const BADGES = [
  {
    key: "legendar",
    label: "LEGENDAR",
    className: "bg-sunset-orange/15 text-sunset-orange border-sunset-orange/40",
  },
  {
    key: "murit",
    label: "AM MURIT",
    className: "bg-sunset-amber/15 text-sunset-amber border-sunset-amber/40",
  },
  {
    key: "fail",
    label: "FAIL",
    className: "bg-foreground/8 text-foreground/70 border-foreground/15",
  },
  {
    key: "wow",
    label: "WOW",
    className: "bg-sunset-magenta/15 text-sunset-magenta border-sunset-magenta/40",
  },
] as const;
    ...
    <div className="pb-32 max-w-[480px] mx-auto bg-[#050505] min-h-screen text-white" style={hind}>
      ...
      <header className="flex items-center justify-between px-4 pt-5 pb-3 sticky top-0 z-30 bg-[#050505]/85">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <svg
              viewBox="0 0 24 24"
              className="size-5 fill-none stroke-white/80"
              strokeWidth="1.5"
              strokeLinecap="round"
      ...
      {isLoading ? (
        <div className="space-y-4 pt-4 px-3">
          {[0, 1].map((i) => (
            <div key={i} className="aspect-square rounded-2xl bg-foreground/[0.04] animate-pulse" />
          ))}
        </div>
      ) : !data || sortedItems.length === 0 ? (
        <div className="pt-4 space-y-5 px-3">
          {promoCards.length > 0 && (
            <SponsoredFazaCard key={`ad-empty-${promoCards[0].id}`} ad={promoCards[0]} />
          )}
        ...
        <div className="pt-4 space-y-5 px-3">
          {sortedItems.map((it, idx) => {
            ...
            const ad = showAd
              ? promoCards[(Math.floor((idx + 1) / 3) - 1) % promoCards.length]
              : null;
            ...
            const isVideo =
              it.media_type === "video" || /\.(mp4|webm|mov|m4v)(\?.*)?$/i.test(it.photo_url);
            const isLiked = data.likedSet.has(it.id);
            const isReposted = data.repostedSet.has(it.id);
            ...
            const article = (
              <article
                key={it.id}
                className="relative rounded-3xl overflow-hidden bg-[#111] border border-white/5 shadow-[0_4px_24px_-12px_rgba(0,0,0,0.8)]"
              >
                      ...
                      >
                        <div className="size-6 rounded-full overflow-hidden border border-[#ff3d8b]">
                          {profile?.avatar_url ? (
                            ...
                            <div className="size-full bg-white/10 grid place-items-center text-[10px] uppercase font-bold">
                  ...
                  <div className="absolute inset-x-3 bottom-3">
                    <div className=" bg-black/35 border border-white/10 rounded-2xl p-2.5 flex items-center justify-around">
                      ...
                      >
                        <svg
                          viewBox="0 0 24 24"
                          className={`size-6 ${isLiked ? "fill-[#c724ff] stroke-[#c724ff]" : "fill-none stroke-white/90"}`}
                          strokeWidth="1.8"
                          strokeLinecap="round"
                          strokeLinejoin="round"
                        >
                      ...
                      >
                        <svg
                          viewBox="0 0 24 24"
                          className="size-6 fill-none stroke-white/80"
                          strokeWidth="1.8"
                          strokeLinecap="round"
                          strokeLinejoin="round"
                        >
                        ...
                        <span className="text-[10px] font-bold text-white/70">
                          {comments > 0 ? formatCount(comments) : "0"}
                        </span>
                      </button>
                      <button
                        onClick={() => setShareFor(it)}
                        aria-label="Trimite"
                        className="flex flex-col items-center gap-0.5 active:scale-90 transition"
                      >
                        <svg
                      ...
                      <button
                        ...
                        aria-label="Repost"
                        className={`flex flex-col items-center gap-0.5 active:scale-90 transition ${isReposted ? "text-[#ffea00]" : "text-white/80"}`}
                      >
                        <svg
                          viewBox="0 0 24 24"
                  ...
                  {venue?.name && (
                    <div className="flex items-center gap-1.5 mb-2">
                      ...
                      >
                        ...
                        <span className="text-[11px] font-bold uppercase tracking-[0.16em] underline-offset-2 hover:underline">
                          {venue.name}
                        </span>
                      </Link>
                  ...
                  {it.caption ? (
                    <p className="text-sm text-white/90 leading-relaxed font-light">
                      <Link
                        to="/app/user/$id"
                        params={{ id: it.user_id }}
                        className="font-semibold mr-1.5 text-white"
                      >
                        @{handle}
                      </Link>
                      {it.caption}
      ...
      <Link
        to="/app"
        className="block text-center text-[10px] uppercase tracking-widest text-muted-foreground pt-5"
        style={archivo}
      >
        ← înapoi la live
      </Link>
      ...
      {open &&
        ...
        createPortal(<UploadSheet onClose={() => setOpen(false)} />, document.body)}
      {commentsFor &&
        typeof document !== "undefined" &&
        createPortal(
          <CommentsSheet
