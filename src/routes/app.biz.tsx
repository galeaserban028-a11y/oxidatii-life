...
import { Building2, X, Plus, Loader2, Trash2, Eye, MousePointerClick, Megaphone, Image as ImageIcon } from "lucide-react";
...
async function loadCampaigns(businessId: string) {
  const { data } = await supabase
    .from("campaigns")
    .select("id, title, body, status, impressions, clicks, image_urls, starts_at, ends_at, created_at")
...
function BizPage() {
  ...
  const { data: businesses, isLoading } = useQuery({
    queryKey: ["biz-list", user?.id], enabled: !!user, queryFn: () => loadBiz(user!.id),
  });
  ...
  if (!user) return <div className="px-4 pt-10 text-center text-sm text-muted-foreground">Conectează-te ca să promovezi.</div>;
  ...
  if (!isLoading && list.length === 0) {
    return (
      <div className="max-w-md mx-auto px-4 pt-10 pb-24 space-y-6 text-center">
        ...
        <p className="text-sm text-zinc-400">Creează-ți brandul ca să postezi reclame pe ecranul principal al aplicației.</p>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 pt-6 pb-24 space-y-6">
      <header className="rounded-2xl bg-zinc-900/40 border border-white/10 p-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[10px] font-mono uppercase tracking-widest text-zinc-500">promovare · plată per postare</div>
          </div>
        </div>
        <button
      ...
      <div className="grid grid-cols-3 gap-2">
        {TIERS.map((t) => (
          <div key={t.id} className="rounded-2xl border border-white/10 bg-zinc-900/40 p-3 text-center">
            <div className="font-display text-2xl" style={{ color: t.color }}>{t.priceRon}</div>
  ...
  if (!campaigns || campaigns.length === 0) {
    return (
      <div className="rounded-2xl bg-zinc-900/30 border border-dashed border-white/10 p-8 text-center space-y-2">
        ...
        <p className="text-xs text-zinc-600">Apasă „Postare nouă" ca să apari pe ecranul principal.</p>
      </div>
    );
  }
      ...
      {campaigns.map((c: any) => {
        ...
        return (
          <div key={c.id} className="rounded-2xl border border-white/10 bg-zinc-900/40 p-3 flex items-center gap-3">
            {c.image_urls?.[0] ? (
              <img src={c.image_urls[0]} alt="" className="w-16 h-16 rounded-xl object-cover shrink-0" />
            ) : (
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="font-mono text-[10px] text-zinc-500 flex items-center gap-3 mt-1 flex-wrap">
                <span className="flex items-center gap-1"><Eye size={10} /> {c.impressions ?? 0} afișări</span>
                <span className="flex items-center gap-1"><MousePointerClick size={10} /> {c.clicks ?? 0} click-uri</span>
                {expired ? (
                  <span className="text-zinc-600">expirat</span>
                ) : (
                  <span className="text-emerald-400">
                    activ · {(() => { const h = Math.floor(ms / 3_600_000); return h >= 24 ? `${Math.floor(h / 24)}z ${h % 24}h` : `${h}h`; })()}
                  </span>
                )}
              </div>
            </div>
...
function PostModal({ businessId, onClose, onCreated }: {
        ...
        <div className="w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl bg-zinc-950 border border-white/10 overflow-hidden" onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 sticky top-0 bg-zinc-950 z-10">
            <div>
              ...
              <p className="text-[10px] font-mono uppercase tracking-widest text-zinc-500 mt-0.5">apare pe ecranul principal când deschizi aplicația</p>
            </div>
          ...
          <div className="p-5 space-y-4 max-h-[80vh] overflow-y-auto">
            <Field label="Imagine">
              {imageUrl ? (
                ...
                <label className={`flex flex-col items-center justify-center gap-2 cursor-pointer ${inputClass} py-10 border-dashed text-zinc-400 hover:text-white hover:border-neon-crimson/50`}>
            ...
            <Field label="Detalii (opțional)">
              <textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="Linie-up, ora ușilor, ce să porți..."
                ...
                {TIERS.map((t) => (
                  <button key={t.id} type="button" onClick={() => setTier(t.id)}
                    className={`px-2 py-3 rounded-xl text-center border transition ${
                      tier === t.id ? "border-neon-crimson bg-neon-crimson/10" : "border-white/10 hover:border-white/30"
                    }`}>
                    <div className="font-display text-lg" style={{ color: t.color }}>{t.priceRon}</div>
                    <div className="text-[9px] font-mono uppercase tracking-widest text-zinc-500">RON</div>
            ...
            <button disabled={busy} onClick={submit}
              ...
              style={{ background: "var(--gradient-chaos)" }}>
              {busy ? <Loader2 size={14} className="inline animate-spin" /> : `Publică · ${TIERS.find(t=>t.id===tier)!.priceRon} RON`}
            </button>
