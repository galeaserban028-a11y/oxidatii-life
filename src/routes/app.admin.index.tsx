...
function AdminDashboard() {
  ...
  if (isLoading || !data)
    return <div className="p-4 text-sm text-muted-foreground">Se încarcă…</div>;
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[11px] text-muted-foreground">
              Verifică și rezolvă cererile primite
            </div>
          </div>
      ...
      <Section title="Pulsul aplicației" icon={<Activity size={11} />}>
        <div className="grid grid-cols-2 gap-2">
          ...
          <Stat
            ...
            label="Sprits"
            value={data.proofs}
            sub={`+${data.proofs24h} / 24h`}
            accent="#ff3d8b"
          />
          <Stat
            icon={<Wallet size={14} />}
            label="Wallet total"
            value={`${(data.totalWallet / 100).toFixed(0)} RON`}
            sub="în business-uri"
            accent="#00e5ff"
          />
        </div>
      ...
      <Section title="Module admin" icon={<LayoutIcon />}>
        <div className="grid grid-cols-1 gap-2">
          <NavCard
            ...
            accent="#00e5ff"
            title="Useri"
            desc="Acordă coins, schimbă rang, dă admin / mod"
            stat={`${data.users} total`}
            highlight={`+${data.newUsers7d} noi`}
          />
          <NavCard
            ...
            accent="#ff3d8b"
            title="Conținut"
            desc="Faze, sprits, comentarii — moderare"
            stat={`${data.proofs} sprits`}
            highlight={`+${data.proofs24h} azi`}
          />
          <NavCard
            ...
            accent="#00e5ff"
            title="Businesses"
            desc="Conturi business, verificare, tier"
            stat={`${data.businesses} total`}
            highlight={`${data.verifiedBiz} verificate`}
          />
          <NavCard
            ...
            accent="#c724ff"
            title="Campanii"
            desc="Promo, boost-uri, evenimente speciale"
            stat={`${data.campaigns} total`}
            highlight={`${data.activeCamps} active`}
          />
          <NavCard
            to="/app/admin/places"
            icon={<MapPin size={16} />}
            accent="#ffd166"
            title="Locații"
            desc="Adaugă orașe și venues"
            stat={`${data.venues} venues`}
            highlight={`${data.cities} orașe`}
          />
          <NavCard
            to="/app/admin/reports"
            icon={<Flag size={16} />}
            accent={data.openReports > 0 ? "#ff3d8b" : "#888"}
            title="Rapoarte"
            desc="Cereri de la utilizatori"
            stat={`${data.reports} total`}
            highlight={data.openReports > 0 ? `${data.openReports} deschise` : "0 deschise"}
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="font-mono text-[9px] uppercase tracking-widest text-muted-foreground/60 mt-0.5">
          {stat}
        </div>
      </div>
      <ChevronRight
