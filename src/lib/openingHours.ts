...
export function evalOpenNow(raw: any, now = new Date()) {
  ...
  if (yesterday) {
    ...
    if (o != null && c != null && c <= o && nowMin < c)
      return { isOpen: true, closesAt: yesterday.close, opensAt: null as string | null, todayKey };
  }
  if (today) {
  ...
  for (let i = 1; i <= 7; i++) {
    ...
    const s = hours[k];
    if (s)
      return { isOpen: false, closesAt: null, opensAt: `${DAY_LABELS[k]} ${s.open}`, todayKey };
  }
